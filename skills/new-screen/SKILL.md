...
2. **Follow the php-patterns skill** for the file layout and handler structure: endpoints under `public/{feature}/` (`index.php`, `form.php`, `save.php`, `delete.php`), query functions in `app/features/{feature}/queries.php`, views under `app/views/{feature}/` (`page.php` + `partials/`). One screen = the pattern's prescribed files, no more.
